...
function Results() {
  ...
  return (
    <div>
      <h2 className='block text-2xl tracking-tight font-extrabold text-gray-900 sm:text-3xl'>Total results: {items.length}</h2>
      <div className='mt-4'>
        <ul role="list" className="space-y-4">
          {items.map((item) => (
            <li key={item.id} className="bg-white shadow overflow-hidden rounded-md px-4 py-4 xl:px-6 2xl:py-6">
